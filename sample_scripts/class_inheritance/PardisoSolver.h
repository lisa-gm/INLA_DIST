#ifndef PARDIDSOSOLVER_H
#define PARDIDSOSOLVER_H
...
class PardisoSolver: public Solver {
   public:
      PardisoSolver(){
         std::cout << "constructing pardiso solver." << std::endl;
      }

      void symbolic_factorization(SpMat& Q) {
         std::cout << "do some magic." << std::endl;
      }

      void factorize(SpMat& Q) {
         std::cout << "rows(Q) = " << Q.cols() << std::endl; 
      }

      void factorize_solve(SpMat&Q, Vector& x) {
         x.setZero();
      }

      void selected_inversion(SpMat&Q, Vector& inv_diag) {
         inv_diag = 2*Vector::Ones(inv_diag.size());
      }
};
...
public:
    ...
    void symbolic_factorization(SpMat& Q, int& init){
        ...
        std::cout << "successfully completed symbolic factorization!" << std::endl;
