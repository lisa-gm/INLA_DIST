...
//{
    ...
    for(int c=0; c<1; c++){
		...
		Qx.reserve(Qst.nonZeros() + Qss.nonZeros());
        ...
        // allocate memory
        ia_Qx = new long unsigned int [nst + nss + 1];
        ja_Qx = new long unsigned int [nnz_Qx];
        a_Qx  = new double [nnz_Qx];

        for (i = 0; i < nst+nss+1; ++i){
        ...
        flops_Qx_factorize = solver_Qx->factorize(ia_Qx, ja_Qx, a_Qx);
...
//{
    ...
    //for(int c=0; c<1; c++){
    	...
    	t_factorise = get_time(0.0);
        flops_factorize = solver->factorize(ia, ja, a);
