...
PostTheta::PostTheta(int ns_, int nt_, int nb_, int no_, SpMat Ax_, Vect y_, SpMat c0_, SpMat g1_, SpMat g2_, SpMat g3_, SpMat M0_, SpMat M1_, SpMat M2_, Vect theta_prior_param_, string solver_type_, const bool constr_, const MatrixXd Dx_, const MatrixXd Dxy_, const bool validate_, const Vect w_) : ns(ns_), nt(nt_), nb(nb_), no(no_), Ax(Ax_), y(y_), c0(c0_), g1(g1_), g2(g2_), g3(g3_), M0(M0_), M1(M1_), M2(M2_), theta_prior_param(theta_prior_param_), solver_type(solver_type_), constr(constr_), Dx(Dx_), Dxy(Dxy_), validate(validate_), w(w_)  {
	...
	// set up PardisoSolver class in constructor 
	// to be independent of BFGS loop
	threads_level1 = 1; //omp_get_max_threads();
	//threads_level2 = 32;

	// set number of threads now!
	omp_set_nested(1);

	double active_levels = 2;
	omp_set_max_active_levels(active_levels);
 	
   	threads_level2 = std::floor(omp_get_max_threads()/threads_level1);
...
//#endif
    ...
    if((MPI_rank) == 0){
    	...
    	std::ofstream log_file(log_file_name);
    	log_file << "MPI_rank threads_level1 threads_level 2 iter_count t_Ftheta_ext t_thread_nom t_priorHyp t_priorLat t_priorLatAMat t_priorLatChol t_likel t_thread_denom t_condLat t_condLatAMat t_condLatChol t_condLatSolve" << std::endl;
    	log_file.close();
    }	
#endif
...
double PostTheta::eval_post_theta(Vect& theta, Vect& mu){
	...
	} // end pragma omp for thread 0, evaluating nominator
	else
	{
	...
	t_thread_denom += omp_get_wtime();
#endif
	} // end else, thread 1, evaluate denominator
