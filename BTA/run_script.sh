#mpirun -np 2 ./BTASolver 42 3 data/A_126_126_ns42_nt3.dat
...
#folder_path=/home/x_gaedkelb/BTA/data/ns${ns}
...
echo "GPU 0 ./main ${ns} ${nt} ${nb} ${no} ${folder_path} ${solver_type} #>BTA_output.txt"

echo "GPU 0 ./mainEigen ${ns} ${nt} ${nb} ${no} ${folder_path} ${solver_type} #>BTA_output.txt"
#CUDA_VISIBLE_DEVICES="0" ./mainEigen ${ns} ${nt} ${nb} ${no} ${folder_path} ${solver_type} #>BTA_output.txt

#srun ./main ${folder_path} ${ns} ${nt} ${nb} ${no} # >${folder_path}/BTA_output.txt
CUDA_VISIBLE_DEVICES="0" ./mainEigen ${ns} ${nt} ${nb} ${no} ${folder_path} ${solver_type} #>BTA_output_s${ns}_nt${nt}_nb${nb}_magmaNative_i2.txt
#mv ${folder_path}/L_factor_BTA_ns${ns}_nt${nt}_nb${nb}_no${no}.dat ${folder_path}/L_factor_BTA_ns${ns}_nt${nt}_nb${nb}_no${no}_gpu0.dat
#mv ${folder_path_data}/x_sol_BTA_ns${ns}_nt${nt}_nb${nb}_no${no}_year${year}.dat ${folder_path_data}/x_sol_BTA_ns${ns}_nt${nt}_nb${nb}_no${no}_year${year}_gpu0.dat
