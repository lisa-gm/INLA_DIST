...
public:
	...
	PostTheta(int ns_, int nt_, int nb_, int no_, SpMat Ax_, VectorXd y_, SpMat c0_, SpMat g1_, SpMat g2_, SpMat g3_, SpMat M0_, SpMat M1_, SpMat M2_) : ns(ns_), nt(nt_), nb(nb_), no(no_), Ax(Ax_), y(y_), c0(c0_), g1(g1_), g2(g2_), g3(g3_), M0(M0_), M1(M1_), M2(M2_)  {
	    ...
	    // create as many Solvers as min(dim_grad_loop, threads)
	    /*#pragma omp parallel for
	    for(int i=0; i<num_solvers; i++){
	        solverQst[i] = new PardisoSolver();  // for spatial-temporal precison matrix only
	       	solverQ[i]   = new PardisoSolver();	 // for conditional precision matrix of denominator
	    }*/
     ...
     * @param [out] mu vector of the conditional mean
     */	
     ...
     * @param [in] theta hyperparameter vector
     * @param [out] cov covariance matrix of the hyperparameters
     */	
	...
	void eval_denominator(Vector& theta, double& log_det, double& val, SpMat& Q, Vector& rhs, Vector& mu){
		...
		/*double old_log_det;
		solve_pardiso(Q, rhs, mu, old_log_det);

		if(abs(log_det - old_log_det) > 1e-6){
			std::cout << "ERROR : log determinants not equal!" << std::endl;
			std::cout << "log det old : " << old_log_det << std::endl;
			std::cout << "log det new : " << log_det << std::endl;
