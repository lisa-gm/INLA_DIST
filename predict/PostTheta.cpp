...
void PostTheta::get_fullFact_marginals_f(Vect& theta, SpMat& Qinv){
	...
	if(constr == true){
		std::cout << "in getFullFact marginals with constraints. not implemented yet." << std::endl;
		...
		if(omp_get_thread_num() == 1 || threads_level1 == 1)
		{
			...
			// iterate through the nonzero entries of Qinv and update each entry

			Vect update_vars(n);
			for(int i=0; i<n; i++){
				update_vars[i] = V.row(i)*S.col(i); // this needs to be computed for all non-zero entries in Qinv ...??
			}
			...
			//std::cout << "update_vars = " << update_vars.tail(10).transpose() << std::endl;
			Qinv = Qinv - VS;
			Qinv = Qinv.selfadjointView<Lower>();
			//std::cout << "vars        = " << vars.tail(10).transpose() << std::endl;			
		}
		}
...
void PostTheta::compute_fullInverseQ(Vect& theta, MatrixXd& Qinv){
	...
	if(omp_get_thread_num() == 1 || threads_level1 == 1)
	{
		solverQ->compute_full_inverse(Q, Qinv);
	}
	}

#ifdef PRINT_TIMES
		timespent_sel_inv_pardiso += omp_get_wtime();
		std::cout << "time spent in inversion pardiso : " << timespent_sel_inv_pardiso << std::endl; 
#endif	
...
void PostTheta::construct_Q_spat_temp(Vect& theta, SpMat& Qst){
	...
	if(MPI_rank == 0){
	    ...
	    int min_size = min(10, nt);
	    std::cout << "M0  : \n" << M0.block(0,0,min_size,min_size) << std::endl;
	    std::cout << "M1  : \n" << M1.block(0,0,min_size,min_size) << std::endl;
	    std::cout << "M2  : \n" << M2.block(0,0,min_size,min_size) << std::endl;
	   }
#endif
