...
#folder_path=/home/hpc/ihpc/ihpc060h/b_INLA/data/${data_type}/ns${ns}_ntFit${nt_fit}_ntPred0_noPerTs${no_per_ts}_nss${nss}_nb${nb}
...
echo "srun -n ${num_ranks} ./call_INLA ${ns} ${nss} ${nt_fit} ${nt_pred} ${nt_total} ${no_per_ts} ${nb} ${folder_path} ${solver_type}" 
srun -n ${num_ranks} ./call_INLA ${ns} ${nss} ${nt_fit} ${nt_pred} ${nt_total} ${no_per_ts} ${nb} ${folder_path} ${solver_type} >output_ns${ns}_nss${nss}_nt${nt_total}_nb${nb}_${solver_type}_testMergedV.txt #>INLA_RGF_output_ns${ns}_nt${nt}_nb${nb}_${num_ranks}_${l1t}_${l2t}_newRGFClass_nestedOMP_singleCopyV_i1.txt
